...
export const AuthContextProvider = ({ children }) => {
  ...
  const registerUser = useCallback(
    async (e) => {
      ...
      ); // it is a promise that is called when a state changes and it is a dependency array
  ...
  async (e) => {
    ...
    if (response.error || typeof response.message !== "undefined" || typeof response === "string") {
      let customMessage =
        typeof response === "string"
          ? response
  ...
  ); //it is a provider that is used to provide user data from the server and error messages from the server to the children components
};
